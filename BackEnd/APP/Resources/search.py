...
class Search(Resource):
    ...
    def _arguments(self, args: dict):
        ...
        if len(args.get('Country')) != 0:
            perfect_dict["Country"] = {'$in':list(set(args.get('Country')))}
        ...
        if args.get('category') != None and args.get('category') != "":
            perfect_dict["Industry"] = args.get('category')
        ...
        if(args.get('keyword')!=""):
            keyword = [
                {'JobTitle':{'$regex':args.get('keyword')}},
                {'AssetName':{'$regex':args.get('keyword')}},
                {'CampaignName':{'$regex':args.get('keyword')}},
                {'CompanyName':{'$regex':args.get('keyword')}},
                {'Industry':{'$regex':args.get('keyword')}},
    ...
    def post(self):
        ...
        try:
            ...
            response = Mongodb.Aggregation(pipe)
            print(pipe)
            output = list()
            for i in response:
                output.append(i)
